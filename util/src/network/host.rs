	...
	fn session_readable(&self, token: StreamToken, io: &IoContext<NetworkIoMessage<Message>>) {
		...
		if let Some(session) = session.clone() {
			...
			match s.readable(io, &self.info.read().unwrap()) {
				...
				Ok(SessionData::Packet { data, protocol, packet_id }) => {
    ...
    where Message: Send + Sync + Clone + 'static,
{
	...
	fn message(&self, io: &IoContext<NetworkIoMessage<Message>>, message: &NetworkIoMessage<Message>) {
		match *message {
			NetworkIoMessage::AddHandler { ref handler, ref protocol, ref versions } => {
				let h = handler.clone();
			...
			NetworkIoMessage::AddTimer { ref protocol, ref delay, ref token } => {
				let handler_token = {
